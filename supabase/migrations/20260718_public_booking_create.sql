create or replace function public.normalize_public_booking_phone(value text)
returns text
language sql
immutable
as $$
  select case
    when regexp_replace(coalesce(value, ''), '\D', '', 'g') ~ '^5[0-9]{9}$'
      then '+90' || regexp_replace(coalesce(value, ''), '\D', '', 'g')
    when regexp_replace(coalesce(value, ''), '\D', '', 'g') ~ '^05[0-9]{9}$'
      then '+90' || substring(regexp_replace(coalesce(value, ''), '\D', '', 'g') from 2)
    when regexp_replace(coalesce(value, ''), '\D', '', 'g') ~ '^90[0-9]{10}$'
