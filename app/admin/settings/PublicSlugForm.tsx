...
export function PublicSlugForm({ initialSlug }: PublicSlugFormProps) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok || !payload.success) {
        setError(payload.error ?? "Public randevu bağlantısı güncellenemedi.");
        return;
      }

      const nextSlug = payload.data?.public_slug ?? normalizePublicSlug(slug);
      setSlug(nextSlug);
      setMessage("Public randevu bağlantısı güncellendi.");
      router.refresh();
    } catch {
      setError("Public randevu bağlantısı güncellenemedi.");
    } finally {
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="space-y-2">
        ...
        <p className="rounded-lg border border-blue-500/20 bg-blue-500/10 px-3 py-2 text-sm text-blue-100">
          Önizleme: <span className="font-medium">/book/{normalizedPreview || "isletme-adi"}</span>
        </p>
      </div>
