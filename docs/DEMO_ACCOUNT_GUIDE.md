...
- Türkçe veya İngilizce demo için `DEMO_LOCALE=tr|en`
...
İngilizce demo için `DEMO_LOCALE="en"` kullan. Bu değer organization profilinde varsayılan email/UI locale bilgisini de hazırlar.
...
- `organization_profiles` kaydını, public slug ve `default_locale` değerini hazırlar.
...
5. `organization_profiles` içine demo organization için ad, public slug ve `default_locale` ekle.
...
İngilizce hizmet/personel/müşteri demo verisi için `DEMO_LOCALE="en"` kullan.
...
- İngilizce public booking: `/en/book/<demo-slug>`
